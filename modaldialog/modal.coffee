...
class Modal extends DOMView
    ...
    triggerSubmit: (e) =>
        ...
        if @currentView and @currentView.beforeSubmit
            return if @currentView.beforeSubmit(e) is false

        if !@submit and !@currentView?.submit and !@getOption('submitEl')
    ...
    next: (options = {}) ->
        if @currentIndex+1 < @views.length
            @openAt(_.extend(options, {_index: @currentIndex+1}))

    previous: (options = {}) ->
        if @currentIndex-1 < @views.length-1
            @openAt(_.extend(options, {_index: @currentIndex-1}))
